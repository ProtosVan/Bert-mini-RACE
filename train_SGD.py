...
if __name__ == "__main__":
    ...
    if args.resume_training :
        ...
        bertForRace = BertForRace(model, config, args.dropout_rate)
        bertForRace.load_state_dict(checkpoint['model_state_dict'])
        if args.lock_para:
            optimizer=torch.optim.SGD([bertForRace.fc1.weight, bertForRace.fc1.bias, bertForRace.fc2.weight, bertForRace.fc2.bias], lr=args.learning_rate, momentum=0.9, weight_decay=args.weight_decay)
        else:
            optimizer=torch.optim.SGD(bertForRace.parameters(), lr=args.learning_rate, momentum=0.9, weight_decay=args.weight_decay)
    ...
    train_dataset = read_dataset([args.dataset_path + "/train_h/data.json", args.dataset_path + "/train_m/data.json", args.dataset_path + "/dev_h/data.json", args.dataset_path + "/dev_m/data.json"])
    train_dataloader = data.DataLoader(train_dataset, batch_size=batch_size)
    
    dev_dataset = read_dataset([args.dataset_path + "/test_h/data.json", args.dataset_path + "/test_m/data.json"])
    ...
    for epoch in range(resume_epoch, args.max_epoch):
        ...
        print("[TRAIN] Epoch %d, train loss:%f, train accu:%f"%(epoch, loss_sum/len(train_dataloader), accu/len(train_dataset)))
        logger.info("[TRAIN] Epoch %d, train loss:%f, train accu:%f"%(epoch, loss_sum/len(train_dataloader), accu/len(train_dataset)))
        ...
        with tqdm(total=len(dev_dataloader), desc='Dev', leave=True, unit='batch', unit_scale=True) as pbar:
            for step, (index, tokens, labels) in enumerate(dev_dataloader):
                ...
                pbar.update(1)
        print("[TEST] Epoch %d, test loss:%f, test accu:%f"%(epoch, test_loss_sum/len(dev_dataloader), test_accu/len(dev_dataset)))
        logger.info("[TEST] Epoch %d, test loss:%f, test accu:%f"%(epoch, test_loss_sum/len(dev_dataloader), test_accu/len(dev_dataset)))
        
        if test_accu/len(dev_dataset) >= max_accu:
            max_accu = test_accu/len(dev_dataset)
